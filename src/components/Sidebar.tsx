...
export function Sidebar({ report, view, onSelect, query, onQueryChange }: SidebarProps) {
    ...
    const visibleThreads = report.threads.filter((thread) => threadMatchesQuery(thread, query))
                ...
                {visibleThreads.length === 0 ? (
                    ...
                    visibleThreads.map((thread) => (
                        <ThreadItem
                            key={thread.index}
                            thread={thread}
                            active={view.kind === 'thread' && view.threadIndex === thread.index}
                            onClick={() => onSelect({ kind: 'thread', threadIndex: thread.index })}
                        />
                    ))
                )}
            </ul>
...
function sameView(a: View, b: View): boolean {
    ...
    if (a.kind === 'thread' && b.kind === 'thread') return a.threadIndex === b.threadIndex
    return true
}
